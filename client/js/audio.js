...
define(['area'], function(Area) {
    ...
    var AudioManager = Class.extend({
        init: function(game) {
            ...
            this.soundNames = ["loot", "hit1", "hit2", "hurt", "heal", "chat", "revive", "death", "firefox", "achievement",
                               "kill1", "kill2", "noloot", "teleport", "chest", "npc", "npc-end", "lever", "levelup", "firecast",
                               "icecast", "healcast", "fireimpact", "iceimpact", "placebomb","arrowshot",
                               "terrorcast","terrorimpact","stuncast","stunimpact","poisoncast","poisonimpact","blackholecast","blackholeimpact","transformcast","transformimpact"];
        ...
        toggle: function() {
            ...
            if(this.enabled) {
                this.enabled = false;
				this.game.storage.setMute(true);
				this.game.storage.save();

                if(this.currentMusic) {
                    this.resetMusic(this.currentMusic);
                }
                $('#mutebutton.main_cell .menuicon').removeClass('active');
            } else {
                this.enabled = true;
				this.game.storage.setMute(false);
				this.game.storage.save();
                ...
                this.updateMusic();
                $('#mutebutton.main_cell .menuicon').addClass('active');
            }
        },
		
		mute: function(muteAudio){
			if(muteAudio) {
                this.enabled = false;
				this.game.storage.setMute(true);
				this.game.storage.save();
        ...
        load: function (basePath, name, loaded_callback, channels) {
            var path = this.game.cdn+basePath + name + "." + this.extension+'?='+this.game.version,
