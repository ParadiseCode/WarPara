...
define(['jquery', 'animation', 'spritelist'], function($, Animation, SpriteList) {

    var ANIMATION_SEQUENCE = {Armor:"armorAnimation", Mob:"mobAnimations", CIA:"collectItemsAnimations"};

    var Sprite = Class.extend({
        init: function(name, scale, cdn, version) {
            this.name = name;
            this.scale = scale;
			this.cdn = cdn;
        ...
        loadData: function() {
            var data = this.tryLoadSequenceByType();
            this.id = data.id;
        ...
        tryLoadSequenceByType: function() {
            for(var key in ANIMATION_SEQUENCE) {
                var isUnUniqueAnimation = (this.name.substr(0, key.length) == key);
                var dataSource = SpriteList[this.name];

                if (isUnUniqueAnimation) {
                    var result = SpriteList[ANIMATION_SEQUENCE[key]];

                    if(MobList.isMob(MobList.getMobIdByName(this.name)))
                        dataSource = MobList.getAnimation(MobList.getMobIdByName(this.name));

                    result.id = dataSource.id;

                    for (var param in dataSource)
                        result[param] = dataSource[param];
