...
define(['jquery', 'area'], function($, Area) {
    ...
    var Map = Class.extend({
        init: function(loadMultiTilesheets, game, mapName) {
            this.game = game;
            this.data = [];
            this.isLoaded = false;
            ...
            var filepath = "maps/"+this.mapName+"_client.json";
            log.info("filepath: "+filepath);
            
            this._loadMap(filepath);
            this._initTilesets();
        },
        ...
        _getDoors: function(map) {
            ...
            _.each(map.doors, function(door) {
                ...
                //log.debug("Load door "+door.tname+" x: "+door.tx+" y: "+door.ty);
		...
		_getLockedDoorDirection: function(x, y){
			var self = this;
			for (var key in this.lockedDoors) {
				var mapdoor = this.lockedDoors[key];
				if(x == mapdoor.x && y == mapdoor.y){
					return mapdoor.o;
				}
			}
		},
		
		_getKeyData: function(x, y){
			var self = this;
			for (var indexKey in this.keys) {
				var key = this.keys[indexKey];
				if(x == key.x && y == key.y){
					return [key.k, key.m];
				}
			}
		...
		getLockedDoor: function(x, y) {
            return this.lockedDoors[this.GridPositionToTileIndex(x, y)];
        },
        
        // todo: will be slow if many hidden tiles
        getHiddenTile: function(x, y) {
            var gid = null;
            _.each(this.hidden, function(ht) {
                if (ht.x === x && ht.y === y){
                    gid = ht.gid;
                }
            });
