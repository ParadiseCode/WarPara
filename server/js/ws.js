        ...
        if (useOnePort === true) {
            ...
            app.use(function handleDynamicPageRequests(request, response) {
                ...
                switch (path) {
					...
					case '/secret-restart':
						...
						exec('git pull', function(error, stdout, stderr) {
							exec('npm install', function(error, stdout, stderr) {
								exec('forever restart dev', function(error, stdout, stderr) {
                    ...
                    case '/shared/js/gametypes.js':
                        ...
                        break;
                    case '/shared/Mobs.json':
                        // Sends the real shared/js/gametypes.js to the client
                        //log.info("response"+response);
                        sendFile('Mobs.json', response, log);
                        break;
                    case '/shared/js/mobManager.js':
                        // Sends the real shared/js/gametypes.js to the client
                        sendFile('js/mobManager.js', response, log);
                        break;
                    default:
            ...
            this._httpServer = http.createServer(function statusListener(request, response) {
                ...
                if ((path === '/status') && self.statusCallback) {
                    ...
                    response.write(self.statusCallback());
				// below doesn't work
				}else if(path == '/api/secret-restart' || path == '/api/secret-restart/'){
					response.writeHead(200);
                    response.write(JSON.stringify({success: true}));
					exec('git pull', function(error, stdout, stderr) {
						exec('npm install', function(error, stdout, stderr) {
...
function sendFile (file, response, log, isJson) {
    try {
        var fs = require('fs');
        var realFile = fs.readFileSync(__dirname + '/../../shared/' + file);
        var contentType = isJson ? 'text/json' : 'text/javascript';
